...
const AdminForgotPassword = () => {
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    ...
    try {
      await adminAuthService.forgotPassword({ email: values.email, turnstileToken });
  ...
  return (
    <AuthLayout
      variant="admin"
      title={sentTo ? 'Check your inbox' : 'Reset your staff password'}
      subtitle={
        sentTo
          ? undefined
          : 'Enter your work email address and we will send a 6-digit reset code.'
      }
      footer={
        <>
          Got your password?{' '}
      ...
      {sentTo ? (
        ...
        >
          ...
          <Button
            ...
            onClick={() => navigate(`/admin/reset-password?email=${encodeURIComponent(sentTo)}`)}
          >
            Enter the code
          </Button>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
          ...
          <Turnstile onVerify={setTurnstileToken} action="admin-forgot-password" />
